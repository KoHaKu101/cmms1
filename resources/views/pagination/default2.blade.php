    ...
    @if ($paginator->hasPages())
     <nav aria-label="Page navigation">
      ...
      <ul class="pagination">
        ...
        @foreach ($elements as $element)
          ...
          @if (is_array($element))
            @foreach ($element as $page => $url)
              ...
              @else
              <li class="page-item "><a href="{{ $url.'&SEARCH='.($search ?? request()->search) .'' }}" class="page-link" aria-label="Goto page {{ $page }}">{{ $page }}</a></li>
              @endif
            @endforeach
          @endif
        @endforeach
        ...
        <li class="page-item text-primary"><a class="page-link" href="{{ $paginator->nextPageUrl() }}">Next</a></li>
        @else
