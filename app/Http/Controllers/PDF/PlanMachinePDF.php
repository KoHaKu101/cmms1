...
class PlanMachinePDF extends Fpdf
{
  ...
  public function PdfPlanPm(){
    ...
    $DateEnd = $date->addDays($daysToAdd)->format('Y-m-d');
      ...
      $logo = "assets/img/logo13.jpg";
      // Logo
      $this->pdf->Cell(26,22,$this->pdf->Image($logo,12,11,22),1,0,'C',false);
      // font
      $this->pdf->AddFont('THSarabunNew','','THSarabunNew.php');
      $this->pdf->AddFont('THSarabunNew','B','THSarabunNew_b.php');
      // Arial bold 15
      ...
      $this->pdf->text(141, 16, iconv('UTF-8', 'cp874', 'Checked '),1,'C');
      $this->pdf->text(165, 16, iconv('UTF-8', 'cp874', 'Issued '),1,'C');
      $this->pdf->text(183, 16, iconv('UTF-8', 'cp874', 'Rev : Date '),1,'C');
      $this->pdf->Cell(22, 22, iconv('UTF-8', 'cp874', ''),1,0,'C');
      $this->pdf->Cell(22, 22, iconv('UTF-8', 'cp874', ''),1,0,'C');
      $this->pdf->Cell(22, 22, iconv('UTF-8', 'cp874', ''),1,0,'C');
      $this->pdf->Cell(22, 22, iconv('UTF-8', 'cp874', ''),1,'C');



      // header
      $this->pdf->SetFont('THSarabunNew','',12);
      $this->pdf->text(190,5,'Page '.$this->pdf->PageNo().'/{nb}',0,0,'R');
      $this->pdf->Ln(1);
    //  $this->Rect( 15,  49, 40, 11, 'D');
    //   $this->SetXY( 15,  54);

      $this->pdf->SetFont('THSarabunNew','',10);
      $cel=array(8,20,35,15,10,10,10,10,8,8,11);
      $rHigeht=8;
